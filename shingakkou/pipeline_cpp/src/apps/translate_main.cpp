...
//   02_translate --test        # smoke run from a fresh cache
//   02_translate --file main05
//
// There is no self-test pre-flight; the ctest "UT" suites cover it.
#include <exception>
#include <filesystem>
#include <string>
...
int main(int argc, char** argv) {
    ...
    desc.add_options()
        ...
        ("file", po::value(&only), "translate only this specific script")
        ...
        ("discard-cache", po::bool_switch(&discard_cache),
         "allow --test or --retranslate to delete a cache that already "
         "holds paid work");
    po::variables_map vm;
    ...
    // mistyped option must cost nothing.
    if (auto why = translate::validate_options(batch, test_n)) {
        log_info("[ERROR] " + *why);
        return 2;
    }

    // Created before anything else runs, so a run into a fresh --dir does not
    // fail at the first save.
    ...
    // The table goes in the game ROOT, not Data\.  --out defaults off
    // --game-dir, so it cannot be a static default_value.
    ...
    // both so progress survives a crash or Ctrl+C.  The wipe happens inside
    // translate_all, after the gate, so a refused run destroys nothing.
