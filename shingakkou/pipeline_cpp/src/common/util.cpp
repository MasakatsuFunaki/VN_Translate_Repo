...
// katakana); lead bytes 0x81-0x9F and 0xE0-0xFC with a trail of 0x40-0x7E or
// 0x80-0xFC.  0x80, 0xA0 and 0xFD-0xFF fit none of those shapes -- a naive
// "structurally valid Shift-JIS" test rejects them -- yet CP932 does map them,
// as single characters:
//   0x80 -> U+0080, 0xA0 -> U+F8F0, 0xFD -> U+F8F1, 0xFE -> U+F8F2, 0xFF -> U+F8F3
// MultiByteToWideChar(932) does NOT agree on all five, so they are mapped here
// rather than handed to the OS codec.  Everything else is pre-validated
// structurally and then handed over, letting the OS reject any
// structurally-valid-but-unmapped pair.
...
std::optional<std::string> cp932_to_utf8_strict(const std::uint8_t* p, std::size_t n) {
    ...
    // separated by the five special singles above, which are emitted directly.
...
std::string cp932_to_utf8_replace(const std::uint8_t* p, std::size_t n) {
    // Walk the byte stream emitting U+FFFD per undecodable byte: decode valid
    // runs strictly, slide one byte on failure.
    std::string out;
    std::size_t i = 0;
    while (i < n) {
        std::uint8_t b = p[i];
...
void append_utf16_unit_as_utf8(std::uint16_t u, std::string& out) {
    append_utf8_cp(out, static_cast<char32_t>(u));
}
...
bool is_unicode_space(char32_t cp) {
    ...
    // SPACE is deliberately absent -- it is not whitespace.
    switch (cp) {
