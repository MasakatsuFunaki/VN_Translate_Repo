# Conan 2 recipe for the Shingakkou proxy DLL (winmm.dll).
#
...
#
# Everything is Win32: the profile in profiles/x86 pins arch=x86 and a static
# CRT, because the DLL is injected into a 32-bit DDSystem game process and the
# test binaries have to link against the same object code the DLL ships.
#
# Replaces the former conanfile.txt: a conanfile.txt cannot express a layout,
# and this half needs cmake_layout(build_folder=".") to put the toolchain where
# its preset looks for it.

from conan import ConanFile
from conan.tools.cmake import CMakeDeps, CMakeToolchain, cmake_layout


class ShingakkouProxy(ConanFile):
    name = "shingakkou_proxy"
    version = "1.0.0"
    package_type = "shared-library"
    description = "Shingakkou winmm.dll proxy (runtime text hook)"
