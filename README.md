...
`install/<game>/` holds three things:

| | |
|---|---|
| `bin/` | the executables you run |
| `game/` | drop into the game folder — the proxy DLL and the runtime table |
| `script_output/` | the extraction and its translation |

`bin` and `game` stay apart because each proxy is a 32-bit DLL named after a
system DLL — beside the x64 executables the loader picks it up and then refuses
it. Only the documents at the top of `script_output/` are staged; the CG
working folders under it run to a gigabyte of regenerable bitmaps.
...
CI does not run the tests. `script_output/` is tracked, so the suites that read
it have their fixtures, but others shell out to `TOOLS/`, which is not in the
repository — the gate is the local `--test` run before you tag.

What CI does enforce is that every staged game carries its translation: a
release without it would be a toolchain, not a patch — the download would need
the user's own API budget before the game speaks English.
