...
// 0x80-0xFC.  A naive "structurally valid Shift-JIS" test rejects the five
// singles below, but CP932 maps every one of them:
//   0x80 -> U+0080, 0xA0 -> U+F8F0, 0xFD -> U+F8F1, 0xFE -> U+F8F2, 0xFF -> U+F8F3
// MultiByteToWideChar(932) does NOT accept all of them, so they are mapped
// here rather than handed to the OS codec; the structural pre-validation runs
// first and the OS then rejects any structurally-valid-but-unmapped pair.
...
std::string cp932_to_utf8_replace(const std::uint8_t* p, std::size_t n) {
    ...
    // than skipping to the next NUL) is what keeps a text payload that sits
    // behind a few non-CP932 operand bytes recoverable.
    std::string out;
    std::size_t i = 0;
    while (i < n) {
        std::uint8_t b = p[i];
...
bool is_unicode_space(char32_t cp) {
    ...
    // whitespace, and trimming it would change strings the engine emits.
    switch (cp) {
