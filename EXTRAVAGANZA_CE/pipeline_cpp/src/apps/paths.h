...
//
// Every app resolves its inputs and outputs under one project folder, found
// at run time (see default_project_dir), with `--dir <path>` to override --
// plus `--game-dir <path>` for the game installation, which is elsewhere on
// disk entirely.
#pragma once
...
inline po::options_description common_options(std::string& project_dir,
    ...
    desc.add_options()
        ...
        ("dir", dir, "EXTRAVAGANZA_CE project folder")
        ("game-dir", po::value(&game_dir)->default_value(
             "C:\\Games\\Extravaganza\\EXTRAVAGANZA Complete Edition\\EXTRAVAGANZA_CE"),
