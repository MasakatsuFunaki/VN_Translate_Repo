...
// 00_run_all -- EXTRAVAGANZA ~Mushi Mederu Shoujo~ Complete Edition English
...
//
// The spine is 01_extract -> 02_translate -> deploy: the speaker gate now
// lives inside the translation step, so everything that must happen for the
// game to show English happens inside the command that pays for it.  The
// steps that write the English into the game's own files -- 03_repack,
// 04_translate_xtx, 05_translate_charts -- run between the two.
#include <cstdlib>
#include <exception>
#include <filesystem>
#include <string>
...
// Ship the one artifact the game loads that the pipeline does not write in
// place: the font-patch DLL.  Nothing is rebuilt here -- a rebuild would
...
//
// The DLL is a hard failure: without it the game renders the English at the
// Japanese font size and the word-wrapping the repack step applied no longer
// fits the textbox.
...
int main(int argc, char** argv) {
    ...
    po::options_description desc("00_run_all -- EXTRAVAGANZA CE translation pipeline");
    ...
    log_info(bar);
    log_info("EXTRAVAGANZA ~Mushi Mederu Shoujo~ Complete Edition");
    ...
    for (std::size_t i = 2; i + 1 < steps.size(); ++i) {
        banner(i);
        run_step(steps[i].name, {}, passthrough);
    }
