...
// bytes; a CP932 name is 2 bytes per character and would blow every limit.
...
std::vector<SptString> extract_strings(const Bytes& dec, std::size_t skip_offset) {
    ...
    while (pos < fsize) {
        ...
        // Strict decode only: an undecodable run is opcode data, not text, so
        // it is skipped rather than salvaged.
