...
// Above this share of [NARRATION] among dialogue lines the speaker plate is
...
SpeakerCheckReport run_speaker_checks(const bj::object& extracted) {
    ...
    if (batch.size() < 10) {
        ...
        return fail(report, 3,
                    "only " + std::to_string(batch.size()) +
                        " dialogue lines yielded a real speaker -- the parse is broken");
    }

    print_line("[3/5] reconstructed user-prompt for first 10 speakered dialogue lines:");
    {
        ...
        while (pos < prompt.size()) {
            ...
            // Only the numbered block carries speaker tags; the trailing
            // instructions are prose and must not be scanned.
            if (line.rfind("</lines_to_translate>", 0) == 0) break;
    ...
    if (!name_dict.count(u8"夢美"))
        return fail(report, 4, "the speaker table is missing the core character names");
    print_line(u8"[4/5] the speaker table contains the core character names (夢美 etc.): OK");
