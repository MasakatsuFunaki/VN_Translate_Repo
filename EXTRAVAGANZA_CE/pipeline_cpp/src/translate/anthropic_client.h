...
// rather than giving the batch up.
...
public:
    ...
    // no key is available -- failing here beats failing mid-run.
...
// since that field may hold a shell command) and exports it to the
// environment so child processes inherit it.  Returns the key or "".
