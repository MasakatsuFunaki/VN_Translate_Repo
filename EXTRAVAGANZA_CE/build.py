"""EXTRAVAGANZA_CE build driver — Conan 2 + CMake + MSVC.
...
Release only.  The game (mushiEx_CE.exe, BLACKCyc) is a 32-bit binary, so the
two halves are built for different architectures and that is the trap this
script exists to hide: `pipeline_cpp` is x64 (the tools that read the SPT
archives, call the API and repack), `proxy_dll` is Win32 because it is
injected into the game process as winmm.dll.  One CMake project cannot hold
both, so the root CMakeLists.txt drives each half as a separate sub-build with
its own Conan profile and toolchain.
...
runs them — the pipeline's UT suites and the proxy's SYSTEM tier, which loads
the built DLL into a fake host EXE (never the game itself).  When `--test` and
`--deploy` are given together the tests gate the deploy, so a regression is
never shipped.

Deploying copies `winmm.dll` into the game folder, and that is the whole
payload: unlike the games that ship a translation table next to the EXE, this
one has its English repacked straight into the game's `.spt`, `.xtx` and
`.fxf` files by 03_repack / 04_translate_xtx / 05_translate_charts.  Set
VN_DIST_BUILD to skip the copy, because a distribution build stages the
artifacts itself and may run where no game is installed.
...
# Matches the pipeline's own default (pipeline_cpp/src/apps/paths.h): the DLL
# has to sit beside the game executable, in the same folder the pipeline
# repacks the scripts into.  Override with EXTRAVAGANZA_CE_GAME_DIR.
GAME_DIR = os.environ.get(
    "EXTRAVAGANZA_CE_GAME_DIR",
    r"C:\Games\Extravaganza\EXTRAVAGANZA Complete Edition\EXTRAVAGANZA_CE")
CONFIG = "Release"
...
# cannot span the two.  Order matters — the pipeline produces the repacked
# scripts, and the DLL is what the game loads to render them at a font size
# those scripts are word-wrapped for.
HALVES = [
...
def main():
    parser = argparse.ArgumentParser(
        description="Build the EXTRAVAGANZA_CE pipeline (x64) and proxy DLL (Win32).")
