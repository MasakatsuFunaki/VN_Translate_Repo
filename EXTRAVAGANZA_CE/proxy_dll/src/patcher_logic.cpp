...
{
    // The CPU evaluates `EIP + rel` where EIP, at the moment the
    // displacement is consumed, points at the byte AFTER the 5-byte
    // instruction. So the post-instruction address is the reference
    // point, not the opcode itself.
