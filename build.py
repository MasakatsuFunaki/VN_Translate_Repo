...
`<game>/build.py`, and this script calls that and collects the results.  The
per-game layout is preserved on purpose: `install/<game>/bin` is what you run
and `install/<game>/game` is what you copy into the game folder, and the two
must stay apart because the proxy is 32-bit and carries a system DLL's name.
Flattened beside the x64 executables the loader would pick it up and refuse it.
...
# Names a release must never carry: the extracted script, its translation and
# the runtime tables built from it are the publisher's text.  They are ignored
# by git, but --install copies whatever a game's install target staged, so the
# check belongs here too -- one refactor is all it takes for a game to stage a
# table it deploys and put the script into the artifact.
FORBIDDEN = ("extracted_text", "translated_text", "translation_cache",
             ".tsv")
...
def collect(game):
    """Copy <game>/build/install -> install/<game>, replacing what was there."""
    ...
    print(f"  {game}: {files} file(s) -> {os.path.relpath(dst, ROOT)}")
    return True


def audit_install():
    """Fail the run if any game's text reached the install tree."""
    bad = []
    for base, _, files in os.walk(INSTALL_DIR):
        for f in files:
            low = f.lower()
            if any(m in low for m in FORBIDDEN):
                bad.append(os.path.relpath(os.path.join(base, f), ROOT))
    if bad:
        print("\n[ERROR] the install tree carries game text, which a release "
              "must not ship:")
        for p in bad[:20]:
            print("   " + p)
        if len(bad) > 20:
...
def main():
    ...
    if args.install and not audit_install():
        return 1
    if failed:
