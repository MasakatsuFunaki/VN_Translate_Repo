...
// 0x80-0xFC.  A naive "structurally valid Shift-JIS" test rejects 0x80, 0xA0
// and 0xFD-0xFF, but CP932 maps all five as single characters:
...
// here rather than handed to the OS codec.  Everything else is pre-validated
// structurally -- MB_ERR_INVALID_CHARS on its own is too lenient -- and the OS
// then rejects any structurally-valid-but-unmapped pair.
...
std::string cp932_to_utf8_replace(const std::uint8_t* p, std::size_t n) {
    // One U+FFFD per undecodable byte: decode valid runs strictly, and slide
    // forward a single byte on failure.
    std::string out;
    std::size_t i = 0;
    while (i < n) {
        std::uint8_t b = p[i];
...
bool is_unicode_space(char32_t cp) {
    ...
    // SPACE is deliberately absent: it is a formatting character, and trimming
    // it would key the TSV on a string the engine never emits.
    switch (cp) {
