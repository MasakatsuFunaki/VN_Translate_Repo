...
std::optional<std::string> validate_options(int batch_size, int test_batches) {
    ...
    // A negative cap once read as "no limit" and started an unbounded run --
    // the opposite of what the flag is for.
    if (test_batches < 0)
        return "--test cannot be negative (got " + std::to_string(test_batches) +
               "); use 0 for a full run";
    return std::nullopt;
}
...
int translate_all(const TranslateOptions& opt) {
    ...
    // The gate comes first, before an API key is read and before a client is
    // built: a run that cannot produce a correct answer must cost nothing and
    ...
    write_file_text(opt.output_file, json_pretty(all_data_v, 2));
    char sz[64];
    ...
    // written, so it is built here rather than by a second command someone
    // has to remember: a translation the game never sees is not a
    // translation.
    print_line("");
