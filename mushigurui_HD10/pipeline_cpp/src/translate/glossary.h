...
// Mushigurui HD10 (蟲狂い) character glossary + Anthropic system prompt.
//
// The tables in glossary.cpp are pinned data, not code: changing an entry
// changes every line the model produces from then on and invalidates the
// resume cache, so treat edits as a deliberate re-translation.
#pragma once
