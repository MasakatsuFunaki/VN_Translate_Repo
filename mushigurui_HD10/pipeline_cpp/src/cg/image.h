...
// Image layer for the narrative-CG step: an 8-bit RGB/RGBA raster with decode,
// crop, resize, PNG/BMP/JPEG encode and base64.
//
...
// Anything that is not already RGB or RGBA is promoted to RGBA.  `channels` is
// decided from the CONTAINER HEADER (PNG IHDR colour type, BMP bit count)
// rather than from whatever stb infers -- stb happily reports 3 channels for a
// colour-type-6 PNG whose alpha is fully opaque, and the repack step would then
// write an RGB payload where the game expects RGBA.
