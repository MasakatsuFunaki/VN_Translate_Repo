...
std::string scratch_cache() {
    return (fs::temp_directory_path() / "mgi_ut_cache_io.json").u8string();
}

std::string scratch_purge_cache() {
    return (fs::temp_directory_path() / "mgi_ut_cache_purge.json").u8string();
}
...
TEST(CacheIo, a_cache_that_cannot_be_read_names_the_file) {
    ...
    for (const char* content : NOT_A_CACHE) {
        mgi::write_file(path, std::string(content));
        try {
            tr::load_cache(path);
