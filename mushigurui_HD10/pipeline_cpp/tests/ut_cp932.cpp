...
// CP932 codec tests.
//
// These exist because an earlier decoder asserted that 0x80, 0xA0 and
// 0xFD-0xFF "are never valid lead bytes" and rejected them.  CP932 maps all
// five as single characters, and ~13% of the BLACKCyc games' extracted strings
// contain at least one of them -- so the decoder silently DROPPED those
// strings and every downstream array index shifted.
//
// The whole single-byte table was re-derived by sweep and diffed against this
// build, which is the check that would have caught the bug; the cases below
// pin the bytes that actually appear in the shipped scripts.
...
// Lossy decode: the special singles decode, so they must NOT come back as
// U+FFFD -- only genuinely invalid bytes do.
