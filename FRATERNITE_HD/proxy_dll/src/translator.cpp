...
// IAT-hooks GDI32!TextOutA. On each c=2 call, reads the engine's state
// struct (buf38/buf3c/opcount) to identify the current message, builds
// an EN render plan via BuildRenderPlan, and substitutes each glyph slot
// with the matching ASCII chunk in a narrower font.
...
// Isolated from C++ destructors because __try cannot coexist with them.
...
// FNV-1a 32-bit hash for message identity.
...
// When true, the overlay draws the full EN; the hook blanks body slots.
static bool g_msgHasOverlay   = false;
static int  g_msgSpeakerSlots = 0;

// 3 outline passes + 1 fill pass per glyph slot.
static constexpr int PASSES_PER_GLYPH = 4;

// Auto-narrowed per message: 4 chars/slot default, up to 7 if EN is too long.
static constexpr int MIN_CHARS_PER_SLOT = 4;
static constexpr int MAX_CHARS_PER_SLOT = 7;

// Flat cache keyed by (font height, chars_per_slot). At most ~8 entries.
...
static HFONT GetNarrowFontFor(int height, int cps)
{
    ...
    LOGFONTA lf = {0};
    lf.lfHeight = height;
    // Monospace avoids variable-kerning artifacts at forced narrow widths.
...
static BOOL WINAPI HookedTextOutA(HDC hdc, int x, int y, LPCSTR s, int c)
{
    ...
    // Read engine state struct for the current message.
    ...
    // Snapshot so the hash and JP reconstruction see stable data.
    char snap38[BUF_CAP];
    char snap3c[BUF_CAP];
    ...
    if (h != g_msgPlanHash) {
        // Diagnostic: report out-of-range glyph leaks from the finished message.
        if (g_msgPlanHash != 0) {
        ...
        // Narrow the font only for messages whose EN doesn't fit at MIN.
        ...
        // Full EN line for the overlay; invalid means no TSV match.
        ...
        if (el.valid) {
            ...
            // Clear overlay so the previous EN doesn't linger.
            overlay_toggle::SetCurrentEnglish("", "", h);
        }

        // Corpus capture for the replay regression test.
    ...
    // Advance by call count, not byte-change — repeated identical glyphs
    // (e.g. [81 48] [81 48] [81 48]) would alias to slot 0 otherwise.
    g_msgPlanCallCount++;
    int idx = (g_msgPlanCallCount - 1) / PASSES_PER_GLYPH;
    if (idx > g_msgMaxIdx) g_msgMaxIdx = idx;

    // Record for F1 JP overlay replay.
    overlay_toggle::RecordDialogCall(hdc, x, y, b0, b1, idx, h);

    // No EN for this message — pass the original JP glyph through.
    if (!g_msgHasOverlay) {
        return g_realTextOutA(hdc, x, y, s, c);
    }
    ...
    {
        // Out-of-range glyph — blank it so raw JP doesn't leak.
        g_msgOorGlyphs++;
    ...
    // Overlay draws the full line; suppress the engine's glyph with blanks.
    ...
    // Use a narrower font so multiple ASCII chars fit in one glyph slot.
    TEXTMETRICA tm = {0};
...
// Returns -1 on fault so the driver won't clear spuriously.
