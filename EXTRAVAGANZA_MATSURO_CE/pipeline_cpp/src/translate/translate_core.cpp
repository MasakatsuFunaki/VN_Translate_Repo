...
std::optional<std::string> validate_options(int batch_size, int test_n, int max_batches) {
    ...
    // A negative cap produces budget < 0, which reads as "no limit" and starts
    // an unbounded run -- the opposite of what the flag is for.
    if (max_batches < 0)
...
std::string extract_speaker(const std::string& text) {
    ...
    // after the speaker name.  Without that gate, two-line narrative entries
    // get their leading prose fragment mis-tagged as the speaker.
...
// Returns (translated_count, stopped_early).  `batch_cap` stops the RUN after
// that many API requests (0 = no limit), which is why `api_batches_done` is
// the caller's running total rather than a local: the cap is a spend control,
// and a per-file counter would let a five-file run pay the cap five times
// over.  `log_entries`, when given, collects the per-line smoke log.  Cap and
// log are separate parameters because --max-batches caps without logging and
// --test does both.
std::pair<long long, bool> translate_file(anthropic::LazyClient& client, const std::string& fname,
                                          ...
                                          const std::string& cache_file, int batch_cap,
                                          int& api_batches_done, bj::array* log_entries) {
    ...
    for (std::size_t batch_start = 0; batch_start < translatable.size();
        ...
        if (batch_cap && api_batches_done >= batch_cap) {
            stopped = true;
            break;
        }
    }
...
int translate_all(const TranslateOptions& opt) {
    ...
    // exists: a run that cannot produce a correct answer must cost nothing and
    ...
    if (total_uncached == 0) {
        ...
        // they differ only in whether a per-run smoke log is written, so
        // --test wins when someone passes both.
        const int batch_cap = opt.test_batches > 0 ? opt.test_batches : opt.max_batches;
        const char* cap_flag = opt.test_batches > 0 ? "--test " : "--max-batches ";
        // Counted across the whole run, not per file: the cap is what a
        // capped run is allowed to spend in total.
        ...
        for (const auto& [fname, line_count, uncached_count] : file_stats) {
            ...
            auto [count, stopped] = translate_file(
                ...
                opt.cache_file, batch_cap, api_batches_done,
                opt.test_batches ? &log_entries : nullptr);
            (void)count;
            done_lines += line_count;
            ...
            if (stopped) {
                log_info("\n  " + std::string(cap_flag) + std::to_string(batch_cap) +
                         ": stopping after " + std::to_string(batch_cap) + " batch(es)");
                break;
            }
        }
    ...
    // The runtime table is the last thing the step does, so one invocation
    // produces both the translated document and the table the game reads.
