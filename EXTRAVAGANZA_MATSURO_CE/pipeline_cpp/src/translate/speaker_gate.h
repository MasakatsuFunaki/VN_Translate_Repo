...
//
// EXTRAVAGANZA MATSURO has no speaker field at all.  extract_speaker() parses
// the raw line text, which BLACKCyc stores as "Name\r\n「Dialogue」", and the
// engine renders the first \r\n-segment as the name plate.  The regression
// this catches is the one that costs the most and shows the least: the parse
// stops recognising the prefix, every dialogue line becomes NARRATION, the
// model reads [NARRATION] for all of them, and the answer comes back fluent,
