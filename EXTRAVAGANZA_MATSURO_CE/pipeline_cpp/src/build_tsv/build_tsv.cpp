...
// Split on a literal separator, keeping empty fields: a doubled separator has
// to survive as an empty segment, or blank lines vanish from wrapped text.
...
std::string sanitize_newlines(const std::string& text) {
    // Strip ONLY leading newline characters, never whitespace: a leading space
    // is part of the engine's own indentation and must reach the textbox.
    std::size_t b = 0;
    ...
    for (const auto& fkv : root.get_object()) {
        ...
        for (const auto& sv : strings->get_array()) {
            ...
            if (!utf8_to_cp932_strict(en_text)) {
                // Round-trip through errors='replace' so unmappable chars
