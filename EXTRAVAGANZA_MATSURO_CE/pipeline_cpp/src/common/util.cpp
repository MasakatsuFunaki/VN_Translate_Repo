...
// 0x80-0xFC.  Everything else is structurally invalid and must be rejected --
// MultiByteToWideChar(MB_ERR_INVALID_CHARS) is laxer than that, so the shape
// is pre-validated here and the OS codec is left to reject any
// structurally-valid-but-unmapped pair.
//
// The five exceptions below are real CP932 single characters that a naive
// "structurally valid Shift-JIS" test rejects:
//   0x80 -> U+0080, 0xA0 -> U+F8F0, 0xFD -> U+F8F1, 0xFE -> U+F8F2, 0xFF -> U+F8F3
// MultiByteToWideChar(932) does NOT map all of them, so they are handled here
// rather than handed to the OS codec.
...
std::string cp932_to_utf8_replace(const std::uint8_t* p, std::size_t n) {
    ...
    // valid runs strictly, slide a single byte on failure.  Sliding (rather
    // than skipping to the next NUL) keeps the offsets of everything after a
    // bad byte intact.
    std::string out;
    std::size_t i = 0;
    while (i < n) {
...
bool is_unicode_space(char32_t cp) {
    ...
    // SPACE is deliberately absent -- it is a formatting character, not
    // whitespace, and trimming it would change the string the engine emits.
    switch (cp) {
