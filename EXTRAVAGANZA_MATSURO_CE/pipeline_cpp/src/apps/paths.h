...
//
// Every input and output path is derived from the EXTRAVAGANZA_MATSURO_CE
// project folder, which is found at run time (see default_project_dir) and
// overridable with `--dir <path>`.  `--game-dir <path>` points at the game
// installation (default C:\Games\EXTRAVAGANZA_MATSURO_CE).
#pragma once
...
inline po::options_description common_options(std::string& project_dir,
    ...
    desc.add_options()
        ...
        ("dir", dir, "EXTRAVAGANZA_MATSURO_CE project folder")
        ("game-dir", po::value(&game_dir)->default_value("C:\\Games\\EXTRAVAGANZA_MATSURO_CE"),
