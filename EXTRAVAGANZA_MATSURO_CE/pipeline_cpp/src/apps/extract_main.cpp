...
int main(int argc, char** argv) {
    ...
    const std::string spt_dir = exm::extract::resolve_spt_dir(game);

    exm::log_info("=== Step 1: Extract text from SPT files ===");
    exm::log_info("SPT directory: " + spt_dir + "\n");
    try {
        return exm::extract::run_extract(spt_dir, out_dir + "\\extracted_text.json");
