...
// "are never valid lead bytes" and rejected them.  CP932 decodes all five as
// single characters, and ~13% of this game's extracted strings contain at least
// one of them -- so the decoder silently DROPPED those strings and every
// downstream array index shifted.
//
// The expected values below are the CP932 mappings for those bytes.  The
// cp932_probe app exists for the same reason: a full-byte-space sweep against a
// reference codec re-derives the whole table and diffs it against this build,
// which is the check that would have caught the bug.
...
TEST(Cp932, decodes_the_five_special_single_bytes) {
...
// Still strict where it matters: a lead byte with an out-of-range trail is an
...
// Lossy mode: the special singles now decode, so they must NOT come back as
// U+FFFD.
