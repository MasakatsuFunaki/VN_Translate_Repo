# Conan 2 recipe for the CROSS_CHANNEL proxy DLL (xinput1_3.dll).
#
...
# DLL is injected into cc.exe, a 32-bit WillPlus/AdvHD process.
#
# Replaces the former conanfile.txt: a conanfile.txt cannot express a layout,
# and the layout below is what puts the toolchain where the preset looks.

from conan import ConanFile
from conan.tools.cmake import CMakeDeps, CMakeToolchain, cmake_layout


class CrossChannelProxy(ConanFile):
    name = "cross_channel_proxy"
    version = "1.0.0"
    package_type = "shared-library"
    description = "CROSS-CHANNEL xinput1_3.dll proxy (runtime text hook)"
