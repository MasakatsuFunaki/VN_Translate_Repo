...
// UT — TSV parser tests for CROSS_CHANNEL.
//
...
TEST(ParseTsv, ConvertsUtf8KeyToCp932) {
    TranslationMap m;
    const char tsv[] = "\xE3\x81\x82\thello\n";
    int n = ParseTsvBuffer(tsv, sizeof(tsv) - 1, m);
    EXPECT_EQ(n, 1);
#ifdef _WIN32
    const std::string cp932_a = "\x82\xA0";
    EXPECT_EQ(m.count(cp932_a), 1u);
    EXPECT_EQ(m[cp932_a], "hello");
#else
    EXPECT_EQ(m["\xE3\x81\x82"], "hello");
#endif
}
