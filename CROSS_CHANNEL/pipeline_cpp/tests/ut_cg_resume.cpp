...
// only for a candidate it has not already painted.  A count that disagrees
// with its loop either demands a key the run never uses, or resolves no client
// and lets the per-image handlers report the failure while the run still ends
// successfully.
//
...
const std::vector<cg::ScanKey>& sample_targets() {
    static const std::vector<cg::ScanKey> v = {
        key("sys.cpk", "title"), key("sys.cpk", "menu"), key("ev.cpk", "title"),
        key("ev.cpk", "ev01"),   key("ev.cpk", "ev02"),
    };
    return v;
}
...
const std::string& painted_file() {
    static const std::string path = [] {
        const fs::path p =
            fs::temp_directory_path() / "crc_ut_cg_resume_patched.bmp";
...
const std::string& absent_file() {
    static const std::string path = [] {
        const fs::path p =
            fs::temp_directory_path() / "crc_ut_cg_resume_absent.bmp";
        fs::remove(p);
        return p.u8string();
    }();
    return path;
...
const std::string& patched_dir() {
    static const std::string path = [] {
        const fs::path p = fs::temp_directory_path() / "crc_ut_cg_resume_out";
        fs::remove_all(p);
...
const std::string& path_under_a_moved_root() {
    static const std::string path = [] {
        const fs::path root =
            fs::temp_directory_path() / "crc_ut_cg_resume_old_root";
        fs::remove_all(root);
...
TEST(ResumeCoverage, CoverageIsPerArchiveNotPerEntryName) {
    const cg::ScanKeySet done = {key("sys.cpk", "title")};
    EXPECT_TRUE(cg::needs_scan(done, key("ev.cpk", "title")));
    EXPECT_FALSE(cg::needs_scan(done, key("sys.cpk", "title")));
}
...
TEST(ResumeCoverage, ResumeEntriesOutsideTheTargetsChangeNothing) {
    const cg::ScanKeySet done = {key("retired.cpk", "gone"),
                                 key("retired.cpk", "also_gone")};
...
// One unpainted candidate is work, and work still needs the key.
