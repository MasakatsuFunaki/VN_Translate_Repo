...
// 0xFD-0xFF "are never valid lead bytes" and rejected them.  CP932 defines all
// five as single characters, and ~13% of this game's extracted strings contain
// at least one of them -- so the decoder silently DROPPED those strings and
// every downstream array index shifted.
//
// The expected values below are the CP932 code table's, so a sweep over the
// whole single-byte range against this build is the check that would have
// caught the bug.
...
TEST(Cp932, special_bytes_survive_inside_a_run) {
    // 'B5 FF FF FF 20 03' -- the shape opcode operands take in sn.bin.
...
// U+FFFD -- only genuinely undecodable bytes are replaced.
