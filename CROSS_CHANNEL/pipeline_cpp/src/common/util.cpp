...
// 0x80-0xFC.  Validating that structure ourselves keeps strictness under our
// control -- MultiByteToWideChar(MB_ERR_INVALID_CHARS) is inconsistent at the
// edges -- and the OS codec then rejects any structurally-valid-but-unmapped
// pair.
//
// The five singles below are exactly where the OS codec and CP932 disagree:
//   0x80 -> U+0080, 0xA0 -> U+F8F0, 0xFD -> U+F8F1, 0xFE -> U+F8F2, 0xFF -> U+F8F3
// A naive "structurally valid Shift-JIS" test rejects all five, which drops
// every string containing one, so they are mapped here rather than handed to
// the OS codec.
...
bool is_unicode_space(char32_t cp) {
    ...
    // would change a TSV key the engine emits verbatim.
    switch (cp) {
